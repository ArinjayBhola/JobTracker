...
export function JobTrackerClient({ initialEntries }: { initialEntries: JobEntry[] }) {
  ...
  const stats = {
    ...
    pipeline: activeEntries.filter(
      (e) =>
        e.applicationStatus === "Draft" || e.applicationStatus === "Applied" || e.applicationStatus === "UnderReview",
    ).length,
    interviews: activeEntries.filter((e) => e.applicationStatus === "InterviewScheduled").length,
    followups: activeEntries.filter((e) => {
      ...
      if (e.opportunityType === "ColdEmail" || e.opportunityType === "LinkedInDM") {
        return true;
      }
    ...
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <main className="mx-auto w-full max-w-7xl px-4 sm:px-6 py-8 space-y-8 animate-fade-in">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">Track applications, interviews, and follow-ups.</p>
        </div>
