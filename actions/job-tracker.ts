  ...
  switch (filter) {
      ...
      return await db
        ...
        .where(
          and(
            eq(jobTracker.userId, userId),
            isActive,
            inArray(jobTracker.applicationStatus, ["Draft", "Applied", "UnderReview"]),
          ),
        )
        .orderBy(desc(jobTracker.createdAt));
    ...
    case "interviews":
      return await db
        .select()
        .from(jobTracker)
        .where(and(eq(jobTracker.userId, userId), isActive, eq(jobTracker.applicationStatus, "InterviewScheduled")))
        .orderBy(desc(jobTracker.createdAt));
    ...
    case "ghosted":
      return await db
        .select()
        .from(jobTracker)
        .where(and(eq(jobTracker.userId, userId), isActive, eq(jobTracker.responseStatus, "Ghosted")))
        .orderBy(desc(jobTracker.createdAt));
    ...
    case "archived":
      return await db
        .select()
        .from(jobTracker)
        .where(and(eq(jobTracker.userId, userId), eq(jobTracker.isArchived, "true")))
        .orderBy(desc(jobTracker.createdAt));
